import os, shutil, time, lmdb, numpy as np
...
def list_array_names(env):
    out = []
    print(f"Start list_array_names: {os.path.basename(env.path())}")
    count = 0
    with env.begin() as txn:
        for k, _ in txn.cursor():
            ...
            if count % 100 == 0:
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] scanned {count} keys from {os.path.basename(env.path())}")
            count += 1
    out = sorted(set(out))
    if not out:
...
def merge_many(src_dirs_all, dst_dir):
    src_dirs = [d for d in tqdm(src_dirs_all, desc=f"scan -> {os.path.basename(dst_dir)}", unit="dir") if os.path.isdir(d)]
    if not src_dirs:
    ...
    try:
        ...
        for e in tqdm(envs, desc=f"read meta -> {os.path.basename(dst_dir)}", unit="lmdb"):
        ...
        totalN = sum(Ns)
        pbar = tqdm(total=totalN, desc=f"merge -> {os.path.basename(dst_dir)}", unit="row")
        offset = 0
...
# -------- chunkwise --------
print('Begin merging chunkwise data...')
cw_src_all = [os.path.join(BASE, f"ODE_Causal_chunkwise/merged_multi_ckpt/merged_20260409105433_74000_part{i:02d}") for i in range(36)]
cw_dst = os.path.join(BASE, "ODE_Causal_chunkwise/merged_multi_ckpt/merged_20260409105433_74000")

...
def set_seq_frame_len(args, caller: str = "") -> None:
    ...
    image_or_video_shape = getattr(args, "image_or_video_shape", [1, 21, 48, 80, 44])
    frame_len = int((image_or_video_shape[-2] * image_or_video_shape[-1]) / (2 * 2))
    seq_len = int(image_or_video_shape[1] * frame_len)
    ...
    if seq_len in [32760] and frame_len in [1560]:                  # [1, 21, 16, 60, 104]
        ...
        raise NotImplementedError(f"Unexpected seq_len={seq_len} frame_len={frame_len} from image_or_video_shape={image_or_video_shape}")

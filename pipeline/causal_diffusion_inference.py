...
class CausalDiffusionInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        self.frame_seq_length = get_seq_frame_len(return_seq=False, return_frame=True, caller=str(self.__class__))     # [1, 21, 16, 60, 104] / [1, 21, 48, 30, 52]  # To 2.2
        print(f"CausalDiffusionInferencePipeline: wan_mode={get_wan_version(str(self.__class__))}, self.num_transformer_blocks={self.num_transformer_blocks}, self.frame_seq_length={self.frame_seq_length}")
    ...
    def inference(
        ...
        for current_num_frames in tqdm(all_num_frames):
            ...
            for _, t in enumerate(sample_scheduler.timesteps):
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        if self.local_attn_size != -1:
            ...
            kv_cache_size = get_seq_frame_len(return_seq=True, return_frame=False, caller=str(self.__class__))        # [1, 21, 16, 60, 104] / [1, 21, 48, 30, 52]  # To 2.2

        if get_wan_version(str(self.__class__)) =="2.1":
            num_heads = 12
        elif get_wan_version(str(self.__class__)) =="2.2":
            num_heads = 24  # To 2.2

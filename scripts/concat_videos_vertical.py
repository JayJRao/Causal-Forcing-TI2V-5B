...
def main():
    ...
    parser.add_argument("--tags", nargs="+", default=None, help="Custom label for each model video (must match number of --models); overrides default model name label")
    ...
    if args.tags is not None and len(args.tags) != len(args.models):
        raise ValueError(f"--tags count ({len(args.tags)}) must match --models count ({len(args.models)})")
